...
class ProjectState:
    ...
    -----------
    index: int
        unique index of the state                                       |   This is just an index
    done: FrozenSet[str]
        set containing names of all the tasks completed at the state    |   This has ?indexes? of tasks that must have been completed?
    '''
    index: int
    done: FrozenSet[str]
...
class ProjectNetwork:
    ...
    -----------
    project: Project
        project corresponding the network                               |   Name I suppose?
    network: nx.DiGraph
        directed graph directly representing the network                |   Mystic graph, we probably don't have it :(
    start_done: ProjectState
        an initial node in the network (empty state, nothing done)      |   As said, the starting point
    goal_done: ProjectState
        a goal node in the network (full state, everything done)        |   The finish, last node of the graph
    ...
    edges() -> List[Tuple[ProjectState, ProjectState, Task]]:
        ...
        - task corresponding to the edge        |   probably the cost?
    predecessors(node: ProjectState) -> List[ProjectState]:
        returns all the nodes with an outcoming edge ending at the given node       |   returns all nodes leading here
    successors(node: ProjectState) -> List[ProjectState]:
        returns all the nodes with an incoming edge starting at the given node      |   returns all nodes starting here
    arc_task(start: ProjectState, end: ProjectState) -> Task:
        returns a task corresponding to the edge starting at the given 'start' node |   gives task starting at the given node
        and ending at the given 'end' node                                          |   and ending at another given node
    arc_duration(start: ProjectState, end: ProjectState) -> int:
        returns duration of the task corresponding the edge starting at the given 'start' node  |   as above but gives duratiion instead of ID
