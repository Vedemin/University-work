...
class TypedLambdaEvaluator:
    ...
    def single_step(self, state_before: Term) -> Transition:
        '''
             This method checks whether the term is a value.
        ...
        # - TmLet (TPL book, p. 124), rules: EvalRule.LetV, EvalRule.Let
        # - TmFix (TPL book, p. 144), rules: EvalRule.FixBeta, EvalRule.Fix
        match state_before:
            ...
            case TmLet(_, x, v1, t2) if term_is_val(v1):
                new_state = term_substitute(v1, t2)
                return transition(new_state, EvalRule.LetV)
            case TmLet(fi, x, t1, t2) if not term_is_val(t1):
                witness = self.single_step(t1)
                return transition(TmApp(fi, witness.new_state.term, t2), EvalRule.Let, (witness,))
            # case TmFix(fi, )
            case _:
