...
class LambdaProgram:
    ...
    def _pretty_str(self, context: list[str], term: Term) -> str:
        ...
        match term:
            case TmAbs(_, x, t):
                con, name = self._pick_fresh_name(context, x)
                return "(\\" + name + "." + self._pretty_str(con, t) + ")"
            case TmApp(_, t1, t2):
                return "(" + self._pretty_str(context, t1) + " " + self._pretty_str(context, t2) + ")"
            case TmVar(_, x, cl):
                assert len(context) == cl, "bad context length"
                return self._find_name(context, x)
    ...
    def _pick_fresh_name(self, context: list[str], suggestion: str) -> tuple[list[str], str]:
        ...
        while suggestion in context:
            suggestion += "'"

        new_context = [suggestion]
        new_context += context
        return tuple((new_context, suggestion))
    ...
    def _find_name(self, context: list[str], index: int) -> str:
        ...
        if index < len(context):
            ...
            return self.free_names[index - len(context)]