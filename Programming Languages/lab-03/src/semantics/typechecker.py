...
class TypedLambdaTypechecker:
    ...
    def _typecheck(self, term: Term, type_context: TypeContext) -> LambdaType:
        ...
        match term:
            ...
            case TmZero(): # Nat
                return BaseType.Nat
            case TmIsZero(_, t): # Nat
                t_type = self._typecheck(t, type_context)
                if t_type == BaseType.Nat:
                    return BaseType.Bool
                else:
                    return raise_type_error(LambdaTypeErrorType.UnnaturalArg, t_type)
            case TmSucc(_, t):
                t_type = self._typecheck(t, type_context)
                if t_type == BaseType.Nat:
                    return BaseType.Nat
            ...
            case TmVar(fi, i, _):
                return self._typecheck(i, type_context)
            # case TmApp(fi, t1, t2):
            #     t1_type = self._typecheck(t1, type_context)
            #     t2_type = self._typecheck(t2, type_context)
            #     match t1_type:
