...
class TypedLambdaTypechecker:
            ...
            case TmCase(_, t1, vars, branches):
                ...
                match tyT1:
                    case VariantType(vs):
                        ...
                        #TODO: Change lines below
                        jbt = self._join(branch_types[0], branch_types[1])
                        for i in range(2, len(branch_types)):
                            jbt = self._join(jbt, branch_types[i])
                        return jbt
    ...
    def _is_subtype(self, sub_type: LambdaType, super_type: LambdaType) -> bool:
        ...
        if sub_type == super_type:
            return True
        if super_type == BaseType.Top:
            return True
        match sub_type:
            case ArrowType(sub_left, sub_right):
                match super_type:
                    case ArrowType(super_left, super_right):
                        return self._is_subtype_arrow(sub_left, sub_right, super_left, super_right)
                    case _:
                        return False
            case RecordType(sub_types):
                match super_type:
                    case RecordType(super_types):
                        return self._is_subtype_record(sub_types, super_types)
                    case _:
                        return False
            case VariantType(sub_types):
                match super_type:
                    case VariantType(super_types):
                        return self._is_subtype_variant(sub_types, super_types)
                    case _:
                        return False
        return False
    ...
    def _is_subtype_record(self, sub_cases: OrderedDict[str, LambdaType],
        ...
        for l, T in super_cases.items():
            if l not in sub_cases:
                return False
            else:
                if not self._is_subtype(sub_cases[l], T):
                    return False
        return True
    ...
    def _is_subtype_variant(self, sub_cases: OrderedDict[str, LambdaType],
        ...
        for l, T in sub_cases.items():
            if l not in super_cases:
                return False
            else:
                if not self._is_subtype(T, super_cases[l]):
                    return False
        return True
    ...
    def _join(self, left: LambdaType, right: LambdaType) -> LambdaType:
        ...
        if self._is_subtype(left, right):
            return right
        match left:
            case ArrowType(ll, lr):
                match right:
                    case ArrowType(rl, rr):
                        return self._join_arrows(ll, lr, rl, rr)
                    case _:
                        return BaseType.Top
            case RecordType(types_l):
                match right:
                    case RecordType(types_r):
                        return self._join_records(types_l, types_r)
                    case _:
                        return BaseType.Top
            case VariantType(types_l):
                match right:
                    case VariantType(types_r):
                        return self._join_variants(types_l, types_r)
                    case _:
                        return BaseType.Top
        return BaseType.Top
    ...
    def _join_arrows(self, left_l: LambdaType, left_r: LambdaType,
        ...
        m = self._meet(left_l, right_l)
        j = self._join(left_r, right_r)
        if meet_l is None:
            return BaseType.Top
        else:
            return ArrowType(m, j)
    ...
    def _join_records(self, left_fields: OrderedDict[str, LambdaType],
        ...
        r_types = OrderedDict[str, LambdaType]()

        for l, T in left_fields.items():
            if l in right_fields:
                r_types[l] = self._join(T, right_fields[l])

        return RecordType(r_types)
    ...
    def _join_variants(self, left_cases: OrderedDict[str, LambdaType],
        ...
        v_types = OrderedDict[str, LambdaType]()

        for l, T in left_cases.items():
            if l in right_cases:
                v_types[l] = self._join(T, right_cases[l])
            else:
                v_types[l] = F

        for r, T in right_cases.items():
            if r not in final_dict:
                v_types[r] = T

        return VariantType(v_types)
    ...
    def _meet(self, left: LambdaType, right: LambdaType) -> LambdaType | None:
        ...
        if self._is_subtype(left, right):
            return left
        match left:
            case ArrowType(left_l, left_r):
                match right:
                    case ArrowType(right_l, right_r):
                        return self._meet_arrows(left_l, left_r, right_l, right_r)
                    case _:
                        return None
            case RecordType(types_l):
                match right:
                    case RecordType(types_r):
                        return self._meet_records(types_l, types_r)
                    case _:
                        return None
            case VariantType(types_l):
                match right:
                    case VariantType(types_r):
                        return self._meet_variants(types_l, types_r)
                    case _:
                        return None
        return None
    ...
    def _meet_arrows(self, left_l: LambdaType, left_r: LambdaType,
        ...
        j = self._join(left_l, right_l)
        m = self._meet(left_r, right_r)

        if m is None:
            return None
        else:
            return ArrowType(j, m)
