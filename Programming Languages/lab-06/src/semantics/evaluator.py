...
class TypedLambdaEvaluator:
    ...
    def single_step(self, state_before: LambdaProgramState) -> Transition:
        ...
        # TODO: Fill missing code for:
        # - TmReference, rules: EvalRule.RefV, EvalRule.Ref
        # - TmDereference, rules: EvalRule.DerefLoc, EvalRule.Deref
        # - TmAssignment, rules: EvalRule.Assign, EvalRule.Assign1, EvalRule.Assign2
        # tip. all rules are specified in TAPL, p. 166
        match state_before.term:
            case TmReference(fi, t1):
                t1p = self.single_step(updated_state(t1))
                new_state = t1p.new_state.replace_term(TmReference(fi, t1p.new_state.term))
                return transition(new_state, EvalRule.Ref, (t1p,))
            case TmReference(_, t1) if term_is_val(t1):
                mem, location = state_before.memory.put(t1)
                new_state = updated_state(mem)
                new_state.replace_term(location)
